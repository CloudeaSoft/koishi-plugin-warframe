...
| L-020 | P2 | explore | Add aliases for dual-form key `Orion & Sirius` if Warframe Market lists it separately from `Sirius & Orion` | found while doing L-002 | todo | ExportWarframes has both SiriusSuit and OrionSuit; L-002 only filled Sirius & Orion |
...
| ID | Type | Item | PR |
| --- | --- | --- | --- |
| L-002 | explore | Extend Chinese warframe aliases in `src/warframe/assets/warframeAlias.json` (issue #69) with a spec asserting each alias resolves to exactly one item | [#107](https://github.com/CloudeaSoft/koishi-plugin-warframe/pull/107) |
