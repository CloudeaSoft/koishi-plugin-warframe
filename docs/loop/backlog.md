...
| L-020 | P2 | optimize | Nova alias `加速` matches WFM item `quickening` and Oraxia alias `蜘蛛` matches `arcane_arachne` because exact name lookup runs before warframe aliases | found while doing L-002 | todo | Either drop those nicknames or match aliases before catalog names |
...
| ID | Type | Item | PR |
| --- | --- | --- | --- |
| L-002 | explore | Extend Chinese warframe aliases in `src/warframe/assets/warframeAlias.json` (issue #69) with a spec asserting each alias resolves to exactly one item; nicknames taken from CN community glossaries (Deathcraft/TH-188, WFBotSlang, jsauce, 233乐园/Bilibili), not guessed | branch `cursor/explore-chinese-aliases-a29a` |
